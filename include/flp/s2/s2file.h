...
const char* s2::file::readcstr()
{
	if (m_readData != nullptr) {
		free(m_readData);
	}

	size_t readDataSize = 256;
	m_readData = (char*)malloc(readDataSize);

	size_t i = 0;
	while (!feof((FILE*)m_fh)) {
		if (i >= readDataSize) {
			readDataSize *= 2;
			m_readData = (char*)realloc(m_readData, readDataSize);
		}

		int c = fgetc((FILE*)m_fh);
		m_readData[i++] = (char)c;
